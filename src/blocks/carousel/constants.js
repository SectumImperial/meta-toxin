const WIDTH = 270;
const COUNT = 1;
const ITEMS = 'js-carousel__items';
const ITEM = 'js-carousel__item';
const PREV = 'js-carousel__prev';
const PREV_VISIBLE = 'carousel__prev_visible';
const NEXT = 'js-carousel__next';
const NEXT_VISIBLE = 'carousel__next_visible';
const TOGGLE = 'js-carousel__toggle-knob';
const LINK = 'js-carousel__link';
const CAROUSEL_TOGGLE_ACTIVE = 'carousel__toggle-knob_active';

export {
  WIDTH,
  COUNT,
  ITEMS,
  ITEM,
  PREV,
  NEXT,
  TOGGLE,
  CAROUSEL_TOGGLE_ACTIVE,
  LINK,
  PREV_VISIBLE,
  NEXT_VISIBLE,
};
