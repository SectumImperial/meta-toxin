const MONTHS = [
  'Янв',
  'Фев',
  'Мар',
  'Апр',
  'Май',
  'Июн',
  'Июл',
  'Авг',
  'Сен',
  'Ноя',
  'Дек',
];

const FIELD = 'js-datepicker__field';
const DATEPICKER = 'js-datepicker';
const DATEPICKER_2 = 'js-datepicker_two-fields';
const DATEPICKER_1 = 'js-datepicker_one-field';
const START = 'js-datepicker__field_start';
const END = 'js-datepicker__field_end';
const ACCEPT = 'js-datepicker__button_accept';
const CLEAR = 'js-datepicker__button_clear';
const GROUP = 'js-datepicker__group';
const CONTAINER = 'js-datepicker__container';
const DATES = 'js-datepicker-dates';
const ACTIVE = 'datepicker__container_active-dp';
const CLICKED = 'js-datepicker_clicked';
const LABEL = 'js-datepicker__label';
const ICON = 'js-datepicker__icon';
const DATEPICKER_ICON_ACTIVE = 'datepicker__icon_active';
const NAV_ARROW = '<svg width="17" height="18" viewBox="0 0 17 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.36252 0.984375L16.3781 9L8.36252 17.0156L6.95627 15.6094L12.5344 9.98438H0.346894V8.01562H12.5344L6.95627 2.39062L8.36252 0.984375Z" fill="#BC9CFF"/></svg>';
const PREV = 'prev';
const NEXT = 'next';

const CLEAR_VISIBLE = 'datepicker__button_clear_visible';

export {
  MONTHS,
  FIELD,
  DATEPICKER_2,
  DATEPICKER_1,
  DATEPICKER,
  START,
  END,
  ACCEPT,
  CLEAR,
  GROUP,
  CONTAINER,
  DATES,
  ACTIVE,
  CLICKED,
  LABEL,
  ICON,
  DATEPICKER_ICON_ACTIVE,
  NAV_ARROW,
  PREV,
  NEXT,
  CLEAR_VISIBLE,
};
